...
description: 豆包水印/豆包解析一体包。触发条件：消息中出现豆包分享链接样式（doubao.com/thread/...、dola.com/thread/... 等含 /thread/ 的豆包链接），或提到"豆包""无印豆包""无水印""豆包解析""豆包图片""豆包视频""下载豆包"等关键词时使用。默认走本地网站模式（快速、网页可复用，用户自己点下载）；用户要 agent 自动下载/自动输出文件时走 agent 模式（doubao_engine.py，链路长需等待）。
---
...
一个包、**两种使用方式**，共享同一份解析引擎（`scripts/`，纯 Python 标准库，无需装依赖）：

| 方式 | 说明 | 特点 |
|---|---|---|
| **网站模式（默认）** | 本地网页 `scripts/doubao_web.py`，端口 8765 | 速度快、网页可复用（一次解析反复浏览），**需用户自己点击下载** |
| **Agent 模式** | `scripts/doubao_engine.py`（普通）/ `scripts/doubao_smart.py`（智能过滤） | 链路长、需等待，**可由 agent 自动输出文件到目录** |

Python 路径：`C:/Users/Admin/.workbuddy/binaries/python/versions/3.13.12/python.exe`

## 触发后怎么做（重要：首次加载不需要问用户选哪种方式）

**默认直接走网站模式**，同时用一两句话向用户说明有两种方式，例如：

> 我把解析结果放在本地网站里了（默认方式，速度快，页面可反复用，下载你自己点）。
> 如果想让我直接自动下载输出文件，说一声，我切 agent 模式帮你自动下载（要等一会儿）。

### 网站模式步骤

1. 探测 `http://127.0.0.1:8765/` 是否已在跑（curl 一次，返回 200 即在跑）
2. 没在跑就后台启动：`... scripts/doubao_web.py`（默认 8765），等 2 秒再探测
3. 直接打开带链接参数的页面：`http://127.0.0.1:8765/?url=<分享链接>`（页面会自动开始解析）
4. 告诉用户在网页上：点解析/看大图/单个下载/一键批量 ZIP，时间筛选（30min/1h/6h/12h + 更多下拉 24h/48h/72h）在选项卡上方
5. 手机上访问：页面会显示本机局域网地址提示（iOS 有保存提示条）

### 什么时候切 Agent 模式

用户明确表达以下诉求之一时才切（否则保持网站模式）：
- "你帮我下载"/"自动下载"/"直接输出文件给我"
- 要保存到指定目录、要打包、要转码后交付
- 网站模式用户用不了（如无浏览器环境）
...
| 用户诉求 | 用法 |
|---|---|
| 发来 /thread/ 链接要自动下载（默认） | 普通模式（doubao_engine.py） |
| 提示词只说"要图"/"要视频" | 普通模式 + `--type image/video`；没说则默认都提 |
| "只要今天的 / 最近几小时的" | 普通模式 + `--today` / `--hours N` / `--since "2026-09-17 08:00"` |
| "跳过失败品 / 筛一下质量 / 失败的不要" | 智能过滤模式（doubao_smart.py） |
| 没说清要不要过滤 | 先 scan 报数，再问用户 |
...
# 2. 确认后下载：文件名带时间戳序号（早生成的在前）；--yes 为确认执行标记
... scripts/doubao_engine.py download "<share_url>" --out <目录> --yes \
...
- **网站模式是默认**：首次触发直接开网站，不问用户选哪种；只有用户要"自动下载/输出文件"才切 agent 模式
- **Agent 模式先报数再下载**：任何下载前必须报告"共 N 图 M 视频"并获用户确认
- 直链带签名有时效：scan 后当次下载，过期重跑即可
- 视频默认 H.264 变体（通用兼容）；仅用户要最高清才 `--best`（可能 H.265，部分播放器不支持；网页上会出橙色"转码后下载"按钮）
- 两种方式结果都按生成时间**从旧到新**排序命名（早生成的序号在前）
- 引擎遇"页面未返回可解析数据"会明确报错（风控/网络抖动），重试即可，不要静默当 0 处理
- 8765 端口若被旧进程占用导致改动不生效：杀掉旧 python 进程重启即可
- 仅供学习交流，内容版权归豆包用户所有
...
- 网页端点：`/api/scan`（解析，含 epoch）、`/api/sizes`（异步大小探测）、`/proxy`（带 Range/缓存头）、`/api/zip`（批量打包）、`/api/convert`（H.265→H.264 转码）、`/api/video`
